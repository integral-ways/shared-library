...
public class AspectApiResponse {
    ...
    /**
     * Pointcut that matches all public methods in a class annotated with @RestController.
     */
    ...
    public Object wrapApiResponse(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (result instanceof ResponseEntity<?> responseEntity) {
            ...
            // ...and the body is ALREADY a GeneralApiResponse (e.g., from an exception handler),
            // then do not wrap it again. Just return it as is.
            if (body instanceof GeneralApiResponse) {
                return result;
            }

            // Otherwise, wrap the existing body and return a new ResponseEntity with the original status.
            return ResponseEntity
                    .status(responseEntity.getStatusCode())
                    .body(GeneralApiResponse.success((HttpStatus) responseEntity.getStatusCode(), "Success", body));
        }

        // If the controller returns a raw object, wrap it in our standard success response
